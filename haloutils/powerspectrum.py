...
lib = load_library('libhm', os.path.dirname(__file__))
...
class psargs_t(Structure):
    ...
    """
    z      : float                  # Redshift 
    H0     : float ; h      : float # Hubble parameter (H0 = 100h km/s/Mpc)  
    Om0    : float ; Omh2   : float # Total matter density parameter
    Ob0    : float ; Obh2   : float # Baryon matter density parameter 
    Onu0   : float ; Onuh2  : float # Massive neutrino density parameter
    Ode0   : float                  # Dark-energy density parameter
    w0     : float ; wa     : float # Dark-energy equation of state (w0-wa model)
    ...
    Tcmb0  : float ; theta  : float # Temperature of background radiation (Tcmb0 = 2.7theta K )
    dplus_z: float ; dplus_0: float
