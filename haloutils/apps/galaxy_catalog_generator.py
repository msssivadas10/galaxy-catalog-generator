...
LIB = npct.load_library("libhm", os.path.dirname(os.path.dirname(__file__)))
...
def _prepare_abacus_workspace(
    ...
    # Check if the simulation is available. If available, load the corresponding metadata.
    try:
        meta = get_meta(simname, redshift)
    except ValueError as e:
        logger.exception(f"metadata for simulation {simname!r} at z={redshift} not availbale.")
        return -1
    ...
    # Using the power spectrum table given with the metadata for the catalog generation. This 
    # power spectrum is for z=0 and stored as an astropy Table of `k (h/Mpc)` and `P (Mpc/h)^3`
    # columns. These are converted to values of log(k) and log(P), with values of h and growth 
    # applied.  
    logger.info(f"using CLASS_power_spectrum and Growth available with the metadata...") 
    dplus  = meta["Growth"]
    log_h  = np.log(h)
    pktab  = np.log(structured_to_unstructured(np.array(meta["CLASS_power_spectrum"])))
    pktab += np.array([ log_h, 2*np.log(dplus) - 3*log_h ])
...
def _clean_up(id: int, keep_halos: bool = False) -> None:
    #  Clean up unwanted files, optionally keep halo catalog.

    logger = logging.getLogger()

    files_to_delete = [ f"{id}.meta.dat", f"{id}.vars.dat", f"{id}.gbuf.dat" ]
    if not keep_halos: files_to_delete.append(f"{id}.hbuf.dat")
...
def galaxy_catalog_generator(
    ...
    # Using the process ID as default for sharing
    ...
    _clean_up(process_id, keep_halos = False)
...
if __name__ == "__main__":
    ...
    def get_log_filename(): 
        log_filesdir = os.path.join( os.getcwd() , "logs" )
        log_filename = os.path.join( log_filesdir, os.path.basename(__file__).rsplit('.', 1)[0] + ".log" )
        os.makedirs(log_filesdir, exist_ok = True)
        return log_filename
    ...
    options = [
        ...
        ("--m0"          , float                      , "Halo model parameter M0 (Msun)"           , -1.        ),
        ("--sigma-m"     , float                      , "Halo model parameter sigmaM"              ,  0.        ),
        ("--alpha"       , float                      , "Halo model parameter alpha"               ,  1.        ),
        ("--scale_shmf"  , float                      , "Scale parameter for subhalo mass-function",  0.5       ),
        ("--slope_shmf"  , float                      , "Slope parameter for subhalo mass-function",  2.        ),
        ("--filter_fn"   , Choice(["tophat", "gauss"]), "Filter function for variance calculation" , "tophat"   ),
        ...
        ("--rseed"       , int                        , "Random seed value"                        , None       ),
    ]
    cli = galaxy_catalog_generator
    for option, otype, help, *default in reversed(options):
        if not default:
            cli = click.option(option, type = otype, required = True, help = help)(cli)
        else:
            cli = click.option(option, type = otype, default = default[0], help = help)(cli)
